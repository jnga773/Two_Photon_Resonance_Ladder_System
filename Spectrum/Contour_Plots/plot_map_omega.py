...
def data(directory_f="./data_files/map_omega/"):
    """
...
#                       Data values                                           #
###############################################################################
directory = "./data_files/map_omega/"
ext = ".txt"
file = "spectrum"
...
tau = np.genfromtxt(fname=directory + time_name + ext, dtype='float', usecols=(0,))

xi = float(open(directory + param_name + ext).readlines()[0])
alpha = float(open(directory + param_name + ext).readlines()[1])

omega = np.genfromtxt(fname=directory + param_name + ext, dtype='float',
                      skip_header=3, usecols=(0,))
...
###############################################################################
#om = omega[10] # for a given delta
#n = np.where(omega == om)[0][0] # the index where delt is in delta list
#delt = delta[n]
#corr = squeezed(corr_mat[n, :]) # Taking a slice of the data
#spec, wlist = spectrum(tau, corr)
#
### Plot the correlation function
##plt.plot(tau, corr.real)
##plt.xlabel(r'$\tau$')
...
#plt.title(r'$\Omega = %s, \delta=%s, \xi=%s$'%(om, delt, xi))
#
#plt.tight_layout()
#plt.show()
##plt.savefig("./Images/weak_drive_Omega=%s,xi=%s.png"%(omega,xi), dpi=500)
###############################################################################
...
shift = shiftedColorMap(oldmap, start=0, midpoint=0.175, stop=1.0,
                        name='shifted_cmap')
...
plt.xlabel(r'$\Omega / \gamma$', fontsize=11)
...
plt.tight_layout()
#plt.savefig("../Images/omega_map_delta_15.eps")
#plt.savefig("../Images/omega_map_xi_1_0.eps")
