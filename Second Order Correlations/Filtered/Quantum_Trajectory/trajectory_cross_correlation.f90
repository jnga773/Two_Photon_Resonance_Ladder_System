PROGRAM Two_Filter_Cross_Correlation
  ...
  COMPLEX(KIND=8) :: cas_a
  ! Cavity b Hamiltonian constant
  COMPLEX(KIND=8) :: H_b
  ! Cavity a cascade constant \sqrt{0.5 * \gamma * \kappa_{a}}
  COMPLEX(KIND=8) :: cas_b
  ! Squrare root of half gamma
  REAL(KIND=8) :: sqrt_gamma
  ! Square root of kappa_a
  REAL(KIND=8) :: sqrt_kappa_a
  ! Square root of kappa_b
  REAL(KIND=8) :: sqrt_kappa_b
  ! Square root of gamma * kappa_a
  REAL(KIND=8) :: sqrt_gamma_a
  ! Square root of gamma * kappa_b
  REAL(KIND=8) :: sqrt_gamma_b
  ...
  REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: correlation
  ! Array to store cross correlation
  REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: cross_correlation
  ! File name for saving correlation and cross correlation data
  CHARACTER(LEN=39) :: filename_correlation = "./data_files/trajectory/correlation.txt"
  ! Filename for saving parameters and time
  CHARACTER(LEN=31) :: filename_time = "./data_files/trajectory/tau.txt"
  ! Filename for parameter list
  CHARACTER(LEN=15) :: filename_paramlist = "./ParamList.nml"
  ...
  tau_steps = INT(tau_max / dt)

  ! Allocate dimension for data arrays
  ALLOCATE(correlation(0:tau_steps))
  ALLOCATE(cross_correlation(0:tau_steps))

  ! TRAJECTORY param list
  ! Default value for max time to be integrated
  max_time = 100000

  ! read the TRAJECTORY namelist
  iunit = 31
  OPEN(iunit, FILE=filename_paramlist, STATUS="OLD", DELIM="QUOTE")
  READ(iunit, NML=TRAJECTORY, IOSTAT=istat)
  IF (istat .NE. 0) THEN
    BACKSPACE(iunit)
    READ(iunit, FMT='(A)') line
    CLOSE(iunit)
  ...
  ! Max number of time steps to integrate for
  steps = INT(max_time / dt, KIND=8)

  !----------------------------------------------------------------------------!
  !                    Initialising Hamiltonian Matricse                       !
  !----------------------------------------------------------------------------!
...
!##############################################################################!
    ...
    ! corr_array array for tau_steps time steps.
