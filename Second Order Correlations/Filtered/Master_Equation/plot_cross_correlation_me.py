...
def lorenztian(w_in, w0_in, kappa_in, max_in=0.16):
    """
    docstring
    """
...
# filename
direc = "./data_files/master_equation/"
name = "tau"
ext = ".txt"
...
file = "omega" + str(int(Omega)) + "_xi" + str(int(xi * 10))
ext = ".txt"
...
#
#ax[1].plot(tau, cross_corr, color='k')
...
text_max_a = max_peak_a + (0.02 * max_peak_a)
text_max_b = max_peak_b + (0.02 * max_peak_b)

# filename to save
direct = "./Images/"
name = "omega%s_Da%s_Db%s_kappa%s"%(int(Omega), int(D_a), int(D_b), int(kappa_a))
filename = direct + name

# spectrum
fig, ax = plt.subplots(nrows=1, ncols=1, figsize=[4, 4])
ax.plot(wlist, spec, color='k')
ax.plot(wlist, lorenztian(wlist, D_a, kappa_a, max_peak_a),
        color='k', alpha=0.5, ls='dashed')
ax.plot(wlist, lorenztian(wlist, D_b, kappa_b, max_peak_b),
        color='k', alpha=0.5, ls='dotted')

ax.text(x=D_a-20, y=text_max_a, s='Cavity a', fontsize=11)
ax.text(x=D_b-7, y=text_max_b, s='Cavity b', fontsize=11)
...
fig.tight_layout()
fig.savefig(filename + "_spectrum.pdf")
plt.show()

plt.figure(figsize=[6, 4])
plt.plot(tau, cross_corr, color='k')
#plt.axhline(y=1.0, color='k', alpha=0.5, ls='dashed')
plt.xlabel(r'$ \gamma \tau $', fontsize=11)
plt.ylabel(r'$ g^{(2)}_{\mathrm{Cross}}(\tau) $', fontsize=11)
plt.xlim(-0.05, 10.05)
plt.tight_layout()
plt.savefig(filename + "_g2.pdf")
plt.show()